...
public class SysConfigController extends BaseController {
	...
	public void testConfiguration(
		...
		try {
			...
			SysConfig configBorrowDay = new SysConfig();
			SysConfig configFee = new SysConfig();
			SysConfig configPenaltyFee= new SysConfig();
			for (SysConfig sysConfig : list) {
				...
				if(sysConfig.getCode().equals("fee"))
					configFee = sysConfig;
				else if(sysConfig.getCode().equals("penalty_fee"))
					configPenaltyFee = sysConfig;
				else if(sysConfig.getCode().equals("borrow_day"))
					configBorrowDay = sysConfig;
			...
			Map<String, Object> penaltyFeeValueMap = new HashMap<String, Object>();
			Map<String, Object> feeValueMap = new HashMap<String, Object>();
			String[] borrowDayValue = configBorrowDay.getValue().split(",");
			String[] feeValue = configFee.getValue().split(",");
			String[] penaltyFeeValue = configPenaltyFee.getValue().split(",");
			penaltyFeeValueMap.put("code", "{"+configBorrowDay.getCode()+"}<==>{"+configPenaltyFee.getCode()+"}");
			penaltyFeeValueMap.put("value", "{"+configBorrowDay.getValue()+"}<==>{"+configPenaltyFee.getValue()+"}");
			if(borrowDayValue.length!=penaltyFeeValue.length){
				penaltyFeeValueMap.put("status", "借款天数与预期利率不匹配");
			}else{
				penaltyFeeValueMap.put("status", "借款天数与预期利率配置正常");
			}
			feeValueMap.put("code", "{"+configBorrowDay.getCode()+"}<==>{"+configFee.getCode()+"}");
			feeValueMap.put("value", "{"+configBorrowDay.getValue()+"}<==>{"+configFee.getValue()+"}");
			if(borrowDayValue.length!=feeValue.length){
				feeValueMap.put("status", "借款天数与综合费用不匹配");
			}else{
				feeValueMap.put("status", "借款天数与综合费用配置正常");
			}
			dataList.add(penaltyFeeValueMap);
			dataList.add(feeValueMap);
			...
			for (String code : codes) {
				...
				for (Entry<String, Object> entry : toMap.entrySet()) {
					SysConfig sc = sysConfigService.findByCode(entry.getKey());
					if (null == sc) {
						/*TppBusiness tppBusiness =tppBusinessService.getById(StringUtil.toLong((entry.getKey())));//
						if(null==tppBusiness)//
							continue;
						else
							figValue = tppBusiness.getUrl();//
							figCode = tppBusiness.getId().toString();//
*/					}else{
						figValue = sc.getValue();
						figCode = sc.getCode();
					}
					if (!entry.getValue().toString().contains(figValue)) {
						status++;
						content += DateUtil.dateStr(new Date()) + "【" + code + "】                code:" + entry.getKey()
								+ "       value:" + figValue + "\r\n";
					}
					codeAppen += "{code:" + figCode + "     value:" + figValue + "},  ";
				}
				if (status > 0) {
