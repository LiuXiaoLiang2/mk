...
public class SmsService {
    ...
    public String validateSmsCode(String phone, String type, String vcode) {
        ...
        Map<String, Object> sms = dbService.queryRec("select code, send_time,state,verify_time from sms where phone=? and sms_type=? order by send_time desc limit 1", phone, type);
        if (sms == null) {
            return "未找到对应的验证码记录";
        }
    ...
    public String sendSmsByTpl(HttpServletRequest request, String phone, String smsType, Object... params) {
        Map<String, Object> rec = dbService.queryRec("select * from sms_tpl where type=?", smsType);
        if (rec == null) {
	...
	public void sendSms(String phone, String content, String smsType) {
        ...
        dbService.insert(SqlUtil.buildInsertSqlMap("sms", rec));
