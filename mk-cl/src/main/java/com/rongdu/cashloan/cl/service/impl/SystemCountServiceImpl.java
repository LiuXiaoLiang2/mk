...
import com.rongdu.cashloan.system.domain.SysUserChannelMoney;
 ...
 * @author caitt
 * @version 1.0
 * @date 2017年3月16日上午10:16:51 Copyright 杭州融都科技股份有限公司 现金贷 All Rights Reserved
...
public class SystemCountServiceImpl implements SystemCountService {
	...
	private static String[] address = { "北京市", "上海市", "天津市", "重庆市", "内蒙古自治区", "宁夏回族自治区", "新疆维吾尔自治区", "西藏自治区", "广西壮族自治区",
			"香港特别行政区", "澳门特别行政区", "黑龙江省", "辽宁省", "吉林省", "河北省", "河南省", "湖北省", "湖南省", "山东省", "山西省", "陕西省", "安徽省", "浙江省",
			"江苏省", "福建省", "广东省", "海南省", "四川省", "云南省", "贵州省", "青海省", "甘肃省", "江西省", "台湾省" };
		...
		case 1:
			for (int i = 0; i < address.length; i++) {
				...
				if(sysUserChannelList != null && sysUserChannelList.size() > 0){
					...
					paramMap.put("address", address[i] + "%");
					paramMap.put("list", listForChannelIds);
					String needRepay =  systemCountMapper.sumBorrowAmtByProvinceForChannel(paramMap);
					map.put("value", needRepay);
					//刘晓亮 end
				} else {
					//原来的
		...
		case 2:
			for (int i = 0; i < address.length; i++) {
				...
				if(sysUserChannelList != null && sysUserChannelList.size() > 0){
					...
					paramMap.put("address", address[i] + "%");
					paramMap.put("list", listForChannelIds);
					String borrowRepay = systemCountMapper.sumBorrowRepayByProvinceForChannel(paramMap);
					
					map.put("value", borrowRepay);
					//刘晓亮 end
				} else {
					//原来的
		...
		default:
			for (int i = 0; i < address.length; i++) {
				...
				if(sysUserChannelList != null && sysUserChannelList.size() > 0){
					...
					paramMap.put("address", address[i] + "%");
					paramMap.put("list", listForChannelIds);
					userCount  = systemCountMapper.countRegisterByProvinceForChannel(paramMap);
					//刘晓亮 end
				} else {
					//原来的
